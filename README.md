...
Dialogue auto-plays when entering or advancing a scene. The bundled MP3 always takes priority. These 170 files are locally AI-generated, not human performances; technical validation is separate from human listening and accent approval. If a recording is missing, the line stays readable. Visitors can opt in (Settings, off by default) to a clearly labelled device-voice fallback; if no voice exists for the language, a bilingual notice appears. Some device voices use the browser vendor's online service, unlike the bundled local MP3s. Only fixed story captions are ever spoken, never visitor input. Voice and rain have separate volume controls, and adaptive rain can be switched off. Personal answers never affect the weather.
