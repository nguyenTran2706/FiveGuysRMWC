...
The device-voice fallback depends on the visitor's installed voices; Vietnamese is not guaranteed, and some browser voices (for example Chrome's online voices) send the spoken text to the browser vendor. Only fixed, fictional story captions are spoken this way, never visitor input. Professionally reviewed, locally hosted recorded dialogue is required for dependable literacy access. Provide transcripts and avoid playing entered personal text through a remote speech service without the person's informed choice. Measure first-load transfer size on a production build and representative low-end Android devices; lazy-loaded art, reduced motion, visible focus and contrast should be assessed alongside screen-reader testing.
