...
export default function App() {
  ...
  // Bundled character MP3s take priority; the optional device fallback is off by default.
  ...
  return <div className={`app page-${page} ${reducedMotion ? 'reduce-motion' : ''}`} onPointerDownCapture={() => engine.unlock()} onKeyDownCapture={event => { if (event.key === 'Enter' || event.code === 'Space' || /^[1-9]$/.test(event.key)) engine.unlock(); }}>
    ...
    <header className="site-header">
      ...
      <nav aria-label={language === 'vi' ? 'Điều hướng chính' : 'Main navigation'}><button onClick={scrollAbout}>{t.about}</button><button className={page === 'street' || page === 'game' ? 'active' : ''} onClick={() => navigate('street')}>{t.story}</button><button onClick={beginIntake}>{t.help}<ArrowUpRight /></button><button className={page === 'chat' ? 'active' : ''} onClick={() => navigate('chat')}>{language === 'vi' ? 'Hỏi về quyền' : 'Ask your rights'}</button><button className={page === 'watch' ? 'active' : ''} onClick={() => navigate('watch')}>{watchCopy[language].navLabel}</button></nav>
