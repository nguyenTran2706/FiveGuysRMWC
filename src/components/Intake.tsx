...
import { ArrowLeft, ArrowRight, Check, ChevronRight, FileText, LockKeyhole, Phone, ShieldCheck, Sparkle } from 'lucide-react';
...
export default function Intake({ language, caseFile, onChange, onReview, onBack }: IntakeProps) {
  ...
  function turn(review = false) {
    ...
    if (review || step === questions.length - 1) { onReview(); return; }
  ...
  return <section className="intake-page">
    ...
    <section className="intake-conversation" aria-label={copy.local}>
      ...
      <div className="intake-question-heading"><span className="intake-eyebrow">{copy.question}</span><span>{copy.stepLabels[step]}</span></div><h2 ref={headingRef} tabIndex={-1}>{questions[step]}</h2><p className="intake-question-note">{notes[step]}</p>
      <div className="intake-answer-area" key={step}>
      {step === 0 && <Field label={copy.answer}><textarea autoComplete="off" rows={6} value={caseFile.narrative[narrativeLanguage]} placeholder={copy.narrativePlaceholder} onChange={event => { const value = event.target.value; onChange({ ...caseFile, narrative: { ...caseFile.narrative, [narrativeLanguage]: value } }); setVisaFear(/visa|deport|immigra|di tru|truc xuat|bi duoi ve|so.{0,25}(482|500|gio lam)/.test(normalize(value))); }} /></Field>}
      {step === 1 && <div className="intake-field-stack"><Field label={copy.industry}><input value={caseFile.profile.industry ?? ''} placeholder={copy.industryPlaceholder} onChange={event => updateProfile({ industry: event.target.value })} /></Field><Field label={copy.role}><input value={caseFile.profile.role ?? ''} placeholder={copy.rolePlaceholder} onChange={event => updateProfile({ role: event.target.value })} /></Field></div>}
      {step === 2 && <><div className="intake-option-buttons">{[true, false].map(value => <button key={String(value)} type="button" aria-pressed={caseFile.profile.stillEmployed === value} className={caseFile.profile.stillEmployed === value ? 'intake-option intake-selected' : 'intake-option'} onClick={() => updateProfile({ stillEmployed: caseFile.profile.stillEmployed === value ? undefined : value })}>{value ? copy.stillEmployed : copy.noLongerEmployed}{caseFile.profile.stillEmployed === value ? <Check size={17} /> : <ChevronRight size={17} />}</button>)}</div><Field label={copy.tenure}><input type="number" min="0" max="1000" value={caseFile.profile.tenureMonths ?? ''} placeholder={copy.tenurePlaceholder} onChange={event => updateProfile({ tenureMonths: event.target.value === '' ? undefined : Number(event.target.value) })} /></Field></>}
      {step === 3 && <Field label={copy.visa}><select value={caseFile.profile.visaSubclass ?? ''} onChange={event => updateProfile({ visaSubclass: (event.target.value || undefined) as CaseFile['profile']['visaSubclass'] })}><option value="">{copy.choose}</option>{Object.entries(copy.visaOptions).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></Field>}
      {step === 4 && <EvidenceFields language={language} caseFile={caseFile} onChange={onChange} />}
      {step === 5 && <SafeContactFields language={language} caseFile={caseFile} onChange={onChange} />}
      {step === 6 && <><div className="intake-field-stack"><Field label={copy.alternateName}><input value={caseFile.emergencyContact?.name ?? ''} onChange={event => updateEmergency({ name: event.target.value })} /></Field><Field label={copy.relationship}><input value={caseFile.emergencyContact?.relationship ?? ''} onChange={event => updateEmergency({ relationship: event.target.value })} /></Field><Field label={copy.phone}><input type="tel" value={caseFile.emergencyContact?.phone ?? ''} onChange={event => updateEmergency({ phone: event.target.value })} /></Field></div><label className="intake-check"><input type="checkbox" checked={caseFile.emergencyContact?.mayWeContact === true} onChange={event => updateEmergency({ mayWeContact: event.target.checked })} /><span>{copy.alternateConsent}</span></label></>}
      {step === 7 && <EmployerFields language={language} caseFile={caseFile} onChange={onChange} />}
      {(visaFear || step === 3) && <div className="intake-visa-note"><ShieldCheck size={18} /><div><strong>{copy.visaReassuranceTitle}</strong><p>{copy.visaReassurance}</p><a href={FAIR_WORK_VISA_URL} target="_blank" rel="noopener noreferrer">{copy.visaLink}<ArrowRight size={13} /></a></div></div>}
      </div><div className="intake-form-actions"><button className="intake-text-button" type="button" onClick={() => turn()}>{copy.skip}</button><button className="intake-primary" type="submit">{step === 7 ? copy.review : copy.next}<ArrowRight size={17} /></button></div>
      </form>}
